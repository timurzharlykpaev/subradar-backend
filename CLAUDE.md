...
## 🚨 App Store backward compatibility (КРИТИЧНО)

Mobile app **уже выпущен в App Store** — у пользователей установлены **старые версии**, и они продолжают ходить на этот же `api.subradar.ai`. Адопшен новой версии в App Store идёт постепенно (≈50%/неделя, 90%/4 недели). До тех пор **старые клиенты обязаны продолжать работать**.

**Перед любым изменением API:**

1. **Default to additive.** Новое поведение → новое поле в response, новый эндпоинт (`/v2/...`), новый query-параметр. **Не модифицируй ответ существующего эндпоинта**, который читают старые клиенты.
2. **Новые request-поля = optional с дефолтами**, совпадающими со старым поведением. Старый клиент не шлёт поле → получает ту же семантику что раньше.
3. **Не удаляй / не переименовывай поля**, которые читают старые клиенты. Даже legacy/wrong поля оставляй и заполняй best-effort.
4. **Не удаляй значения enum** (`SubscriptionStatus`, `BillingPeriod`, `Category`) — старые клиенты свитчатся на них и упадут на unknown value.
5. **Не ужесточай DTO-валидацию задним числом** — новое required поле / более строгий regex / более узкий enum в `class-validator` вернут 400 старым клиентам, шлющим прежний payload. Сначала loosen, tighten только после высокого адопшена новой версии.
6. **Server-side gating > client-side gating.** Новые лимиты/правила/планы реализуй на сервере — старые клиенты автоматически получат фикс без обновления.
7. **Если изменение нельзя сделать backward-compat** — предупреди пользователя ЯВНО до коммита: *"Это сломает X на app версиях ≤ A.B.C — продолжать?"*. Не просачивай молча.
8. **FCM push payload + webhook payloads** — те же правила. Новые ключи рядом, старые ключи не удаляй (старые билды свитчатся на них).

**Force-update escape:** если backward-compat реально невозможен (критичный security fix), правильный паттерн — серверный `GET /app/min-version` эндпоинт + клиентский UI который заставляет обновиться. Не casually.

**При запросе на «упрости/удали legacy»**: проверь сначала — это поле точно никто не читает на проде? `git log -p` + grep по mobile-репо до состояния залитого в App Store. Если есть сомнения — оставляй.
